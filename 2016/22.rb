...
DATA.each do |line|
  filesystem, size, used, avail, use_percent = line.split.map do |part|
    if part[0] == ?/
      part.split("-")[1..-1].map { |s| s[1..-1].to_i }
    else
      part.to_i
    end
  end
  x, y = filesystem
  grid[y] ||= []
  grid[y][x] = {:size => size, :used => used, :avail => avail, :can_send_to => [], :can_recv_from => []}
end

width, height = grid[0].length, grid.length
0.upto(height-1).each do |y1|
  0.upto(width-1).each do |x1|
    0.upto(height-1).each do |y2|
      0.upto(width-1).each do |x2|
        unless y1 == y2 and x1 == x2
          if grid[y1][x1][:used] > 0 and grid[y1][x1][:used] <= grid[y2][x2][:avail]
            grid[y1][x1][:can_send_to] << [x2, y2]
            grid[y2][x2][:can_recv_from] << [x1, y1]
          end
        end
      end
    end
...
0.upto(height-1).each do |y|
  0.upto(width-1).each do |x|
    if y == 0 and x == 0
      print "(.)"
    elsif y == 0 and x == width-1
      print " G "
    elsif grid[y][x][:used] == 0
      print " _ "
    elsif grid[y][x][:can_send_to].empty?
      print " # "
    else
      print " . "
    end
