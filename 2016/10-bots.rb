...
DATA.each do |line|
  if line =~ /^value (\d+) goes to bot (\d+)$/
    value, bot = $1.to_i, $2.to_i
    bots[bot] ||= new_bot
    bots[bot][:inv] << value
  elsif line =~ /^bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)$/
    bot, low_loc, low, high_loc, high = $1.to_i, $2, $3.to_i, $4, $5.to_i
    bots[bot] ||= new_bot
    bots[bot][:insts] << [bot, low_loc, low, high_loc, high]
  else
    puts "!!! PARSE ERROR"
  end
end

until bots.all? { |b| b[:insts].empty? }
  bots.each do |bot|
    done = false
    bot[:insts].each do |b, low_loc, low, high_loc, high|
      if bots[b][:inv].length == 2
        if bots[b][:inv].min == 17 and bots[b][:inv].max == 61
          p b
        end
        if low_loc == "bot"
          bots[low] ||= new_bot
          bots[low][:inv] << bots[b][:inv].min
        else
          outputs[low] = bots[b][:inv].min
        end

        if high_loc == "bot"
          bots[high] ||= new_bot
          bots[high][:inv] << bots[b][:inv].max
        else
          outputs[high] = bots[b][:inv].max
        end
...
p (outputs[0] * outputs[1] * outputs[2])
