DATA.each do |line|
  if line =~ /^([a-z\-]+)-(\d+)\[([a-z]{5})\]$/
    code, id, chksum = $1, $2.to_i, $3

    freq = {}
    max = 1
    code.gsub('-', '').chars.each do |ch|
      freq[ch] ||= 0
      freq[ch] += 1

      max = freq[ch] if freq[ch] > max
    end

    real_chksum = ''
    max.downto(1) do |f|
      ?a.upto(?z).each do |ch|
        if freq[ch] == f && !real_chksum.include?(ch)
          real_chksum << ch
          break if real_chksum.length == 5
        end
      end
      break if real_chksum.length == 5
    end
    ...
    if chksum == real_chksum
      ...
      code.chars.each do |ch|
        if ch == ?-
          ...
          decrypted << ((((ch.ord - ?a.ord) + id) % 26) + ?a.ord).chr
        end
      end
      if decrypted == "north pole objects"
        print "!!! "
      end
      puts "##{id}: #{decrypted}"
    end
