...
DATA.each do |line|
  if line =~ /^(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)$/
    op = {'turn on'=>:on, 'turn off'=>:off, 'toggle'=>:toggle}[$1]
    x1, y1, x2, y2 = $2.to_i, $3.to_i, $4.to_i, $5.to_i
    y1.upto(y2) do |y|
      x1.upto(x2) do |x|
        case op
        when :on then rows[y][x] += 1
        when :off then rows[y][x] -= 1 unless rows[y][x] == 0
        when :toggle then rows[y][x] += 2
        end
      end
    end
  else
