...
DATA.each do |line|
  if line =~ /^(\w+) would (gain|lose) (\d+) happiness units by sitting next to (\w+).$/
    People[$1] ||= {}
    People[$1][$4] = $3.to_i * ($2 == "gain" ? 1 : -1)
  else
    puts "!!! PARSE ERROR"
  end
end

def dhappiness(table)
  total = 0
  table.each.with_index do |person, idx|
    next if person.nil?
    total += People[person][table[(idx-1) % table.length]] if table[(idx-1) % table.length]
    total += People[person][table[(idx+1) % table.length]] if table[(idx+1) % table.length]
  end
  total
end
...
list = People.keys
max_dh = -10000
list.permutation.each do |perm|
  dh = dhappiness([nil] + perm)
  if dh > max_dh
    max_dh = dh
  end
end
