...
allergenic_ingredients = ingredients_by_allergen.map { |_, ingredients| ingredients.first }

nonallergenic_ingredients = []
foods.each do |food|
  nonallergenic_ingredients += food.ingredients
end
nonallergenic_ingredients.uniq!
nonallergenic_ingredients -= allergenic_ingredients

sum = foods.sum do |food|
  food.ingredients.count { |ingredient| nonallergenic_ingredients.include?(ingredient) }
end
p sum
