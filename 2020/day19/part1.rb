...
def rule_to_regex(rules, rule_idx)
  rule = rules[rule_idx]
  return rule if rule.is_a? String

  options = rule.map { |option|
    option.map { |sub_rule| '(' + rule_to_regex(rules, sub_rule) + ')' }.join
  }

  '(' + options.join(')|(') + ')'
end

regex = Regexp.new('\A' + rule_to_regex(rules, 0) + '\z')

p messages.count { |message| message =~ regex }
