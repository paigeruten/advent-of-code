rules_str, messages_str = DATA.read.split("\n\n")
...
rules_str.lines.each do |line|
  if line =~ /^(\d+): "(\w)"$/
    ...
    rules[$1.to_i] = $2.split(' | ').map { |part| part.split.map(&:to_i) }
  else
    p "oops"
  end
end
...
# returns every possible number of chars that can be consumed at the beginning
# of the string, by following the given rule.
def parse_message(rules, str, rule_idx = 0)
  rule = rules[rule_idx]

  return [] if str.empty?
  return str[0] == rule ? [1] : [] if rule.is_a? String

  rule.map do |option|
    option.reduce([0]) do |consumed, sub_rule|
      consumed.map do |idx|
        parse_message(rules, str[idx..-1], sub_rule).map { |n| n + idx }
      end.flatten
    end
  end.flatten
end

def matches?(rules, str)
  parse_message(rules, str).include?(str.length)
end

messages = messages_str.lines.map(&:chomp)
