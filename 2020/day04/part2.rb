...
passports = DATA.read.split("\n\n").map { |passport_str|
  Hash[passport_str.split.map { |field| field.split(':') }]
}
...
def valid_year?(str, range)
  str =~ /^\d{4}$/ && range.include?(str.to_i)
end

def valid_height?(str)
  return false unless str =~ /^(\d+)(cm|in)$/
...
def valid_country_id?(str)
  true # >:D
end

def valid_passport?(passport)
  REQUIRED_FIELDS.subset?(Set[*passport.keys]) and
  ...
  valid_hex_color?(passport['hcl']) and
  valid_eye_color?(passport['ecl']) and
  valid_passport_id?(passport['pid']) and
  valid_country_id?(passport['cid'])
end

p passports.count { |passport| valid_passport?(passport) }
