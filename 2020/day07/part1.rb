...
DATA.readlines.each do |line|
  if line =~ /^(.+?) bags? contain (.+?)\.$/
    type, contains = $1, $2.split(', ')
    RULES[type] ||= []
    contains.each do |contained|
      if contained =~ /^(\d+) (.+?) bags?$/
        num, contained_type = $1.to_i, $2
        RULES[type] << [contained_type, num]
      elsif contained == 'no other bags'
      else
        p "oops"
      end
    end
...
def contains_shiny_gold_bag?(bag)
  RULES[bag].any? { |contained_bag, _num|
    contained_bag == 'shiny gold' || contains_shiny_gold_bag?(contained_bag)
  }
end

p RULES.keys.count { |bag| contains_shiny_gold_bag?(bag) }
