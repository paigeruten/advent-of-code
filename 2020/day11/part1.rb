state = DATA.readlines.map(&:chomp)
...
def count_neighbours(state, x, y)
  ...
  count = 0
  count += 1 if y > 0 and x > 0 and state[y-1][x-1] == '#'
  count += 1 if y > 0 and state[y-1][x] == '#'
  count += 1 if y > 0 and x < width-1 and state[y-1][x+1] == '#'
  count += 1 if x > 0 and state[y][x-1] == '#'
  count += 1 if x < width-1 and state[y][x+1] == '#'
  count += 1 if y < height-1 and x > 0 and state[y+1][x-1] == '#'
  count += 1 if y < height-1 and state[y+1][x] == '#'
  count += 1 if y < height-1 and x < width-1 and state[y+1][x+1] == '#'
...
loop do
  ...
  next_state = state.map(&:clone).clone
  (0...state.length).each do |y|
    (0...state[0].length).each do |x|
      num_neighbours = count_neighbours(state, x, y)
      if num_neighbours == 0 and state[y][x] == "L"
        next_state[y][x] = "#"
      elsif num_neighbours >= 4 and state[y][x] == "#"
        next_state[y][x] = "L"
      end
    end
  end
